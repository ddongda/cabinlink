...
唯一来源；消费方在组合根（`Application.onCreate`）把 contract 提供的 `ServiceNode`
**注入**给 Bridge（控制反转，手动 DI / Pure DI，不引入任何 DI 框架）。

## 2. 为何是 DI（控制反转）且不违背铁律

- JSON 方案是 core **主动拉**配置；DI 方案是 contract **推** `ServiceNode` 给 core。
- 依赖方向：`App（组合根）→ :contract:X（构造 ServiceNode）→ :core（暴露注入口）→ :transport`。
- `:contract:X → :core:lite` 已是 `api` 依赖（合法方向），故 contract 可合法构造 core 的
  `ServiceNode`；core 只暴露「接收口」，**永不反向依赖 contract**，铁律不破。

## 3. 关键决策（已确认）

| 决策 | 结论 |
|---|---|
| API 形态 | 并入 `register`：`Bridge.register(ServiceNode, cb)` 一调用完成「连接 + 模块注册 + 就绪回调」 |
| 契约版本 | **并入 `ServiceNode`**（contract 一次性填好），消费方只传描述符 |
| 节点坐标位置 | 新建 `XxxContract` 类承载 `ServiceNode`，`Schema` 仍只管 topic/字段常量 |
| JSON 去留 | **完全移除** `NodeRegistry` + 所有 `assets/bridge_nodes.json` |
| 发现机制 | 编译期静态（contract 常量 + 组合根装配），非反射/PackageManager |

## 4. 组件设计

### 4.1 `ServiceNode`（`:core:lite`，已 public，原 `NodeDescriptor` 重命名）

`NodeDescriptor → ServiceNode`、`id → pkg`；新增 `contractVersion` 字段 + 单模块便捷构造器：
...
register(ServiceNode node)
register(ServiceNode node, ModuleCallback callback)

isReady(String module)   // 不变
```

- **移除** `register(String, ModuleCallback)` 与 `register(String, int, ModuleCallback)`：
  版本已并入描述符，回调与「连接目标（消费方）」配对，统一走 `register(ServiceNode, cb)`。

**`BridgeCore`：**

- `register(ServiceNode node, ModuleCallback cb)`：
  1. 对 `node.modules` 中每个 module：`moduleStates.computeIfAbsent(module, new ModuleState(module, node.contractVersion))`，
     `cb != null` 则加入该模块回调；
  2. `nodeModules.put(node.pkg, node.modules)`（onConnected 归属）；
  3. 若 `node.pkg != selfId`：`connections.connect(node)`（去重）；随后 `worker.execute(reevaluate(module))`。
...
- 新增 `void connect(ServiceNode n)`（原私有 `connect` 提为可被 `BridgeCore` 调用）。
...
- **移除** `connectAll(List<ServiceNode>, selfId)`。
...
- **self 跳过**：`node.pkg == selfId` 不连接（提供方误用 `register(自身NODE)` 也安全）。
...
- 单测（纯 JVM）：
  - `ServiceNode` 单模块构造器：modules 单元素集合、contractVersion 正确。
  - 连接去重逻辑若可抽为纯方法则单测（同一 pkg 二次 register 不重复连接）。
- 真机（既有验证路径）：
